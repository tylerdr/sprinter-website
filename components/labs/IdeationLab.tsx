...
export default function IdeationLab() {
  ...
  const [scattergoriesPrompt, setScattergoriesPrompt] = useState<
    (typeof scattergoriesPrompts)[0] | null
  >(null);
...
Suggestion: Consider adding a social component to increase viral growth potential.`;
    ...
    setIdeas([
      {
        ...
        text: ideaText,
        score: Math.round(
          (scores.originality + scores.feasibility + scores.impact) / 3
        ),
        feedback,
      },
    ]);
  ...
  const playScattergories = async () => {
    const prompt =
      scattergoriesPrompts[
        Math.floor(Math.random() * scattergoriesPrompts.length)
      ];
          ...
          >
            <div className="p-6 rounded-xl bg-card/20 border border-border/30 backdrop-blur-sm">
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-3 bg-brand-gradient text-primary-foreground font-medium rounded-lg hover:opacity-90 disabled:opacity-50 mb-6"
              >
                {isGenerating ? (
                  <>
                ...
                <div className="space-y-3">
                  {ideas.map((idea, index) => (
                    <motion.div
                      ...
                      className="p-4 rounded-lg border border-warning-30 bg-warning-10"
                    >
            ...
            <div className="p-6 rounded-xl bg-card/20 border border-border/30 backdrop-blur-sm">
              ...
              {ideas.length > 0 && ideas[0].feedback && (
                <motion.div
                  ...
                  className="p-6 rounded-lg border border-brand-30 bg-brand-10"
                >
