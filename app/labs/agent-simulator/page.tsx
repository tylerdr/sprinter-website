...
import { Bot, Play, RefreshCw, Zap, Database, Code, CheckCircle, Loader } from "lucide-react"
...
export default function AgentSimulatorPage() {
  ...
  const runSimulation = async () => {
    ...
    const simulatedAgents: Agent[] = [
      { id: "1", name: "Research Agent", icon: Database, status: "working", task: "Gathering information..." },
      { id: "2", name: "Analysis Agent", icon: Code, status: "idle", task: "Waiting for data..." },
      { id: "3", name: "Planning Agent", icon: Zap, status: "idle", task: "Standing by..." },
      { id: "4", name: "Quality Check Agent", icon: CheckCircle, status: "idle", task: "Ready to verify..." },
    ]
    ...
    for (let i = 0; i < simulatedAgents.length; i++) {
      ...
      setAgents(prev => prev.map((agent, index) => {
        if (index === i) {
          return { ...agent, status: "done", result: `Completed: ${agent.task}` }
        } else if (index === i + 1) {
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        >
          <div className="p-6 rounded-xl bg-white/5 border border-white/10 backdrop-blur-sm">
            <label className="block text-sm font-medium mb-3">Give the agents a mission:</label>
            <input
              type="text"
              value={scenario}
        ...
        <AnimatePresence>
          {result && (
            ...
            >
              <h3 className="text-xl font-bold mb-3 text-green-400">✨ Mission Complete!</h3>
        ...
        >
          ...
          <p className="text-gray-400 mb-4">
            This simulation demonstrates how multiple specialized AI agents collaborate to solve complex tasks. 
            Each agent has specific capabilities and they work in parallel, sharing information and coordinating 
            to achieve the goal efficiently.
          </p>
          <p className="text-gray-400">
            In real-world applications, SprinterHQ&apos;s agentic workflows can orchestrate dozens of agents, 
            each handling specific subtasks, calling APIs, processing data, and making decisions autonomously.
          </p>
        </motion.div>
      </div>
    </div>
